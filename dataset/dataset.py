...
class MyDataset(Dataset, Sized):
    ...
    def __getitem__(self, indx):
        ...
        if self._mode != "test":
            ...
            x_mask = _binarize(cv2.imread(join(self._label, imgname + '.png'), cv2.IMREAD_GRAYSCALE))
        else:
            x_ref = cv2.imread(join(self._A, imgname + '.tif'))
            x_test = cv2.imread(join(self._B, imgname + '.tif'))
        ...
        if self._mode != "test":
            ...
            x_ref, x_test = self._to_tensors_test(x_ref, x_test)
            return (x_ref, x_test), imgname
    ...
    def _to_tensors_test(
        self, x_ref: np.ndarray, x_test: np.ndarray) -> Tuple[Tensor, Tensor]:
        return (
            self._normalize(torch.tensor(x_ref.astype(float)).permute(2, 0, 1)),
            self._normalize(torch.tensor(x_test.astype(float)).permute(2, 0, 1)),
        )
...
class MyTestDataset(Dataset, Sized):
    def __init__(
            self,
            data_path: str,
            mode: str,
